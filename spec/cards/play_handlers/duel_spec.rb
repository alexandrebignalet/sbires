RSpec.describe "Duel" do
  ...
  before do
    ...
    @defender_player = @game.players[@game.next_player_index]
    @third_player = @game.players.detect { |p| p != @attacker_player && p != @defender_player }

    pawn_placement
  end

  it "should raise if opponent is not given" do
    expect { @game.draw_card(@attacker_player.lord_name, CardType::GANT) }.to raise_error Sbires::Error
  end

  it "should raise if opponent is the attacker" do
    expect { @game.draw_card(@attacker_player.lord_name, CardType::GANT, opponent: @attacker_player.lord_name) }.to raise_error Sbires::Error
  end

  it "should raise if opponent is unknown" do
    expect { @game.draw_card(@attacker_player.lord_name, CardType::GANT, opponent: "unknown") }.to raise_error Sbires::Error
  end

  context "duel launch" do
    before do
      @game.draw_card(@attacker_player.lord_name, CardType::GANT, opponent: @defender_player.lord_name)
    end
    ...
    it "should only allow current/attacker to roll dice first" do
      expect { @game.roll_dice(@defender_player.lord_name) }.to raise_error Sbires::Error
    end
    ...
    it "should expose succeeded attack number" do
      ...
      dice_result = @game.roll_dice(@attacker_player.lord_name)
    ...
    it "should only allow defender to roll dice after attacker" do
      ...
      @game.roll_dice(@attacker_player.lord_name)

      expect { @game.roll_dice(@attacker_player.lord_name) }.to raise_error Sbires::Error
    end
    ...
    it "should expose succeeded blocks number" do
      ...
      @game.roll_dice(@defender_player.lord_name)

      result = @game.end_turn

      expect(result[:touches]).to eq 3
      expect(result[:blocks]).to eq 1
      expect(result[:winner].is_player?(@attacker_player)).to be true
      expect(result[:loser].is_player?(@defender_player)).to be true
    end
    ...
    it "should give defender a dice number to roll equal to the number of attack touches" do
      ...
      expect(result[:blocks]).to eq 1
      expect(result[:winner].is_player?(@defender_player)).to be true
      expect(result[:loser].is_player?(@attacker_player)).to be true
    end
    ...
    it "should not make defender roll dice if attacker did not make any touches" do
      ...
      expect(@attacker_player.points).to eq 4
      expect(@defender_player.points).to eq 7
    end
    ...
    context "attacker wins" do
      ...
      it "should add 5 points to the attacker" do
        expect(@attacker_player.points).to eq 10
      end

      it "should remove as much attack not blocked as points to the defender" do
        expect(@defender_player.points).to eq 2
      end

      it "should have ended turn" do
        expect(@game.current_player).not_to eq @attacker_player
        expect(@game.current_player).to eq @defender_player
      end
    ...
    context "duel with weapons" do
      it "should not allow any cards except equipment to be played after gant" do
        expect { @game.draw_card(@attacker_player.lord_name, CardType::DEMONSTRATION_MENESTREL) }.to raise_error Sbires::Error
      end
      ...
      it "should allow attacker to draw an equipment card to attack stronger" do
        ...
        @game.draw_card(@attacker_player.lord_name, card_name)

        expect(@attacker_player.cards).not_to include an_object_having_attributes(name: card_name)
      end

      it "should not allow attacker to change equipment when he drew an equipment card" do
        @game.draw_card(@attacker_player.lord_name, CardType::EPEE)
        expect { @game.draw_card(@attacker_player.lord_name, CardType::DAGUE) }.to raise_error Sbires::Error
      end

      it "should allow defender to draw an equipment card to defend better" do
        card_name = CardType::COTTE_DE_MAILLES
        @game.draw_card(@defender_player.lord_name, card_name)

        expect(@defender_player.cards).not_to include an_object_having_attributes(name: card_name)
      end

      it "should not allow defender to change equipment when he drew an equipment card" do
        @game.draw_card(@defender_player.lord_name, CardType::COTTE_DE_MAILLES)
        expect { @game.draw_card(@defender_player.lord_name, CardType::DAGUE) }.to raise_error Sbires::Error
      end
      ...
      it "should not allow attacker to equip after rolling his dices" do
        ...
        expect { @game.draw_card(@attacker_player.lord_name, CardType::EPEE) }.to raise_error Sbires::Error
      end
      ...
      it "should not allow defender to equip after attacker rolled his dices" do
        ...
        expect { @game.draw_card(@defender_player.lord_name, CardType::COTTE_DE_MAILLES) }.to raise_error Sbires::Error
      end
  ...
  def pawn_placement
    @game.place_pawn(@attacker_player.lord_name, NeighbourType::SALLE_D_ARMES)
    @game.place_pawn(@defender_player.lord_name, NeighbourType::SALLE_D_ARMES)
    @game.place_pawn(@third_player.lord_name, NeighbourType::SALLE_D_ARMES)

    @game.place_pawn(@attacker_player.lord_name, NeighbourType::SALLE_D_ARMES)
    @game.place_pawn(@defender_player.lord_name, NeighbourType::EGLISE)
    @game.place_pawn(@third_player.lord_name, NeighbourType::EGLISE)

    @game.place_pawn(@attacker_player.lord_name, NeighbourType::CHATEAU)
    @game.place_pawn(@defender_player.lord_name, NeighbourType::EGLISE)
    ...
    @game.place_pawn(@attacker_player.lord_name, NeighbourType::CHATEAU)
    @game.place_pawn(@defender_player.lord_name, NeighbourType::GRAND_PLACE)
    @game.place_pawn(@third_player.lord_name, NeighbourType::GRAND_PLACE)
    ...
    @game.place_pawn(@attacker_player.lord_name, NeighbourType::TAVERNE)
    @game.place_pawn(@defender_player.lord_name, NeighbourType::GRAND_PLACE)
    @game.place_pawn(@third_player.lord_name, NeighbourType::TAVERNE)

    @game.place_pawn(@attacker_player.lord_name, NeighbourType::GRAND_PLACE)
    @game.place_pawn(@defender_player.lord_name, NeighbourType::TAVERNE)
    @game.place_pawn(@third_player.lord_name, NeighbourType::TAVERNE)
  end
end